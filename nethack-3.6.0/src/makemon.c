...
struct permonst *ptr;
{
    if (ptr->mlet == S_ELEMENTAL)
        ...
        case PM_AIR_ELEMENTAL:
            return uz_is_airlevel();
        case PM_FIRE_ELEMENTAL:
            return uz_is_firelevel();
        case PM_EARTH_ELEMENTAL:
            return uz_is_earthlevel();
        case PM_WATER_ELEMENTAL:
            return uz_is_waterlevel();
        }
    return FALSE;
}
...
register struct monst *mtmp;
{
    ...
    case S_HUMAN:
        if (is_mercenary(ptr)) {
            ...
            if (mm == PM_ELVENKING) {
                if (rn2(3) || (in_mklev && uz_is_earthlevel()))
...
int mmflags;
{
    ...
    *mtmp = zeromonst; /* clear all entries in structure */
    ...
    if ((mcham = pm_to_cham(mndx)) != NON_PM) {
        ...
        if (context.no_of_wizards == 1 && uz_is_earthlevel())
            mitem = SPE_DIG;
...
rndmonst()
{
    ...
    if (rndmonst_state.choice_count < 0) { /* need to recalculate */
        ...
        for ( ; mndx < SPECIAL_PM; mndx++) { /* (`mndx' initialized above) */
            ...
            if (uncommon(mndx))
                continue;
            if (uz_in_hell() && (ptr->geno & G_NOHELL))
                continue;
...
register struct monst *mtmp;
{
    ...
    if (OBJ_AT(mx, my)) {
        ...
        if (mx != 0 && (levl[mx - 1][my].typ == HWALL
            ...
            appear = uz_is_rogue_level() ? S_hwall : S_hcdoor;
        else
