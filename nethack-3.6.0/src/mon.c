...
register struct monst *mtmp;
{
    ...
    if (inlava) {
        ...
        if (mtmp->data->mlet == S_EEL && !uz_is_waterlevel()) {
...
boolean move_other; /* make sure mtmp gets to x, y! so move m_at(x, y) */
{
    ...
    if (move_other && othermon) {
        ...
        if (othermon->mx == 0 && othermon->my == 0) {
            ...
            migrate_to_level(othermon, uz_ledger_no(), MIGR_APPROX_XY, NULL);
        }
    }
...
struct monst *mtmp;
{
    ...
    if (mtmp == u.ustuck) {
        ...
        undetected = (is_pool(x, y) && !uz_is_waterlevel());
...
pick_animal()
{
    ...
    if (uz_is_rogue_level() && !isupper((uchar) mons[res].mlet))
        res = animal_list[rn2(animal_list_count)];
    return res;
}
...
struct monst *mon;
{
    ...
    boolean uppercase_only = uz_is_rogue_level();
