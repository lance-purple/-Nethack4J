...
boolean pushing;
{
    ...
    } else if (!uz_is_waterlevel() && is_pool_or_lava(rx, ry)) {
...
currentlevel_rewrite()
{
    ...
    fd = create_levelfile(uz_ledger_no(), whynot);
    if (fd < 0) {
        /*
...
#ifdef MFLOPPY
    if (!savelev(fd, uz_ledger_no(), COUNT_SAVE)) {
        (void) nhclose(fd);
        delete_levelfile(uz_ledger_no());
...
save_currentstate()
{
    ...
    if (flags.ins_chkpt) {
        ...
        if (fd < 0)
            return;
        bufon(fd);
        savelev(fd, uz_ledger_no(), WRITE_SAVE);
        bclose(fd);
    }
...
boolean at_stairs, falling, portal;
{
    ...
    if (!builds_up(&u.uz)) { /* usual case */
        if (uz_dunlev() > dunlev_reached(&u.uz))
            dunlev_reached(&u.uz) = uz_dunlev();
    } else {
        if (dunlev_reached(&u.uz) == 0
            || uz_dunlev() < dunlev_reached(&u.uz))
            dunlev_reached(&u.uz) = uz_dunlev();
    }
    ...
    if ((mtmp = m_at(u.ux, u.uy)) != 0 && mtmp != u.usteed) {
        ...
        if ((mtmp = m_at(u.ux, u.uy)) != 0) {
            ...
            if (!rloc(mtmp, TRUE))
                ...
                migrate_to_level(mtmp, uz_ledger_no(), MIGR_RANDOM,
                                 (coord *) 0);
        }
    }
