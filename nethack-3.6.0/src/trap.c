            ...
            if (t && otyp == BOULDER) {
                ...
                case TELEP_TRAP:
                    ...
                    if (t->ttyp == TELEP_TRAP)
                        ...
                        if (newlev == uz_depth() || uz_in_endgame())
                            continue;
...
long hmask, emask; /* might cancel timeout */
{
    ...
    if (!trap) {
        ...
        } else if (uz_is_waterlevel() && !no_msg) {
            You_feel("heavier.");
    ...
    assign_level_from_uz(&current_dungeon_level);
    if (trap) {
        switch (trap->ttyp) {
        case STATUE_TRAP:
            break;
...
drown()
{
    ...
    if (!u.uinwater) {
        You("%s into the water%c", uz_is_waterlevel() ? "plunge" : "fall",
            Amphibious || Swimming ? '.' : '!');
        if (!Swimming && !uz_is_waterlevel())
...
crawl:
    if (crawl_ok) {
        ...
        boolean succ = uz_is_waterlevel() ? TRUE : emergency_disrobe(&lost);
    ...
    if (u.uinwater) {
        ...
        You("find yourself back %s.",
            uz_is_waterlevel() ? "in an air bubble" : "on land");
    }
    return TRUE;
}
