...
boolean check_pit;
{
    ...
    return (boolean) ((!Levitation || uz_is_airlevel()
                       || uz_is_waterlevel())
...
register int x, y;
{
    ...
    if (x == u.ux && y == u.uy && u.uswallow && is_animal(u.ustuck->data))
        return "maw";
    else if (IS_AIR(lev->typ) && uz_is_airlevel())
        return "air";
    else if (is_pool(x, y))
        return (Underwater && !uz_is_waterlevel()) ? "bottom" : "water";
    else if (is_ice(x, y))
        return "ice";
    else if (is_lava(x, y))
        return "lava";
        ...
        return "fountain";
    else if ((IS_ROOM(lev->typ) && !uz_is_earthlevel())
...
doengrave()
{
    ...
    if (uz_is_airlevel() || uz_is_waterlevel() /* in bubble */) {
