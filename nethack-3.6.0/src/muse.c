...
#define m_flee(m)                          \
    ...
    case MUSE_WAN_DIGGING: {
        ...
        migrate_to_level(mtmp, uz_ledger_no() + 1, MIGR_RANDOM,
                         (coord *) 0);
        return 2;
    }
    ...
    "As %s climbs the stairs, a mysterious force momentarily surrounds %s...",
            ...
            migrate_to_level(mtmp, uz_ledger_no() - 1, MIGR_STAIRS_DOWN,
                             (coord *) 0);
        }
        return 2;
    case MUSE_DOWNSTAIRS:
        ...
        migrate_to_level(mtmp, uz_ledger_no() + 1, MIGR_STAIRS_UP,
                         (coord *) 0);
        return 2;
    case MUSE_UP_LADDER:
        m_flee(mtmp);
        ...
        migrate_to_level(mtmp, uz_ledger_no() - 1, MIGR_LADDER_DOWN,
                         (coord *) 0);
        return 2;
    case MUSE_DN_LADDER:
        m_flee(mtmp);
        ...
        migrate_to_level(mtmp, uz_ledger_no() + 1, MIGR_LADDER_UP,
                         (coord *) 0);
        return 2;
    case MUSE_SSTAIRS:
        m_flee(mtmp);
...
struct monst *mtmp;
{
    ...
    case MUSE_POT_GAIN_LEVEL:
        mquaffmsg(mtmp, otmp);
        if (otmp->cursed) {
            if (uz_can_rise_up(mtmp->mx, mtmp->my)) {
                register int tolev = uz_depth() - 1;
                d_level tolevel;

                get_level(&tolevel, tolev);
                /* insurance against future changes... */
                if (uz_on_level(&tolevel))
                    goto skipmsg;
                if (vismon) {
