...
dosacrifice()
{
    ...
    highaltar = ((uz_is_astralevel() || Is_sanctum(&u.uz))
...
#define MAXVALUE 24 /* Highest corpse value (besides Wiz) */
    ...
    if (otmp->otyp == AMULET_OF_YENDOR) {
        if (!highaltar) {
        too_soon:
            if (altaralign == A_NONE && uz_in_hell())
    ...
    desecrate_high_altar:
        ...
        if (u.ualign.type != altaralign) {
            ...
            if (ugod_is_angry() || (altaralign == A_NONE && uz_in_hell())) {
...
boolean praying; /* false means no messages should be given */
{
    ...
    if (is_undead(youmonst.data) && !uz_in_hell()
    ...
    return !praying ? (boolean) (p_type == 3 && !uz_in_hell()) : TRUE;
}
