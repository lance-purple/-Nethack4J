...
dokick()
{
    ...
    if (mtmp) {
        ...
        /* recoil if floating */
        if ((uz_is_airlevel() || Levitation) && context.move) {
            int range;
    ...
    if (OBJ_AT(x, y) && (!Levitation || uz_is_airlevel()
                         || uz_is_waterlevel() || sobj_at(BOULDER, x, y))) {
        if (kick_object(x, y)) {
            if (uz_is_airlevel())
    ...
    if (!IS_DOOR(maploc->typ)) {
        ...
        if (IS_THRONE(maploc->typ)) {
            ...
            if ((Luck < 0 || maploc->doormask) && !rn2(3)) {
                ...
                if (uz_dunlev() < dunlevs_in_uz_dungeon()) {
                    fall_through(FALSE);
                    return 1;
                } else
                    goto ouch;
    ...
    dumb:
        ...
        if ((uz_is_airlevel() || Levitation) && rn2(2))
