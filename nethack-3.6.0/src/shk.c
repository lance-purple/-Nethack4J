...
{
    if (uz_on_level(&(ESHK(shkp)->shoplevel)))
...
register boolean silentkops;
{
    ...
    if (!inhishop(shkp)) {
        ...
        if (uz_on_level(&eshkp->shoplevel)) {
            home_shk(shkp, FALSE);
...
int croaked; /* -1: escaped dungeon; 0: quit; 1: died */
{
    ...
    for (mtmp = next_shkp(fmon, FALSE); mtmp;
        ...
        mtmp2 = mtmp->nmon;
        eshkp = ESHK(mtmp);
        local = uz_on_level(&eshkp->shoplevel);
...
coord *mm;
{
    ...
    k_cnt[0] = cnt = abs(uz_depth()) + rnd(5);
