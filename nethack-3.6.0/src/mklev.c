...
make_niches()
{
    int ct = rnd((nroom >> 1) + 1), dep = uz_depth();
...
boolean skip_lvl_checks;
{
    ...
    if (!skip_lvl_checks
        && (In_hell(&u.uz) || In_V_tower(&u.uz) || is_uz_rogue_level()
...
coord *tm;
{
    ...
    if ((kind == TRAPDOOR || kind == HOLE) && !uz_can_fall_thru())
        kind = ROCKTRAP;
...
xchar x, y;
{
    ...
    if (!(u.uz.dnum == oracle_level.dnum      /* in main dungeon */
          ...
          && (u_depth = uz_depth()) > 10    /* beneath 10 */
