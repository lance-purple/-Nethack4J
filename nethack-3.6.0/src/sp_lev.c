...
xchar rtype, rlit;
{
    ...
    if (rlit == -1)
        rlit = (rnd(1 + abs(uz_depth())) < 11 && rn2(77)) ? TRUE : FALSE;
...
struct mkroom *croom;
{
    ...
    if (uz_in_mines() && pm && your_race(pm)
...
struct mkroom *croom;
{
    ...
    if (a->shrine) { /* Is it a shrine  or sanctum? */
        priestini(croom, x, y, (a->shrine > 1));
        ...
        case VAULT:
            for (x = croom->lx; x <= croom->hx; x++)
                for (y = croom->ly; y <= croom->hy; y++)
                    (void) mkgold((long) rn1(abs(uz_depth()) * 100, 51),
                                  x, y);
            break;
        case COURT:
        case ZOO:
...
struct sp_coder *coder;
{
    ...
    if (OV_i(rlit) < 0)
        OV_i(rlit) =
            (rnd(1 + abs(uz_depth())) < 11 && rn2(77)) ? TRUE : FALSE;
